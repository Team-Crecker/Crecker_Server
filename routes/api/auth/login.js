...
router.post('/', async (req, res) => {
    ...
    if (selectUserResult[0] == null) {//id가 존재하지 않으면
        ...
        const hashedEnterPw = await crypto.pbkdf2(password.toString(), salt, 1000, 32, 'SHA512').toString('base64');
        const pa = selectUserResult[0].password
    
        console.log(hashedEnterPw)
        console.log(pa)

        if (selectUserResult[0].password.toString('base64') == hashedEnterPw.toString('base64')) {
